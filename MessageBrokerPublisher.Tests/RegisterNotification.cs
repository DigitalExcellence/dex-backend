...
namespace MessageBrokerPublisher.Tests
{
    public class Tests
    {
        ...
        public void RegisterNotification_EmailNotification_NotificationRegistered()
        {
            ...
            modelMock.Setup(x => x.BasicPublish(It.IsAny<string>(), It.Is<string>(x => x == Subject.EMAIL.ToString()),
                It.Is<bool>(x => x == false),It.IsAny<IBasicProperties>(),
                It.IsAny<ReadOnlyMemory<byte>>())).Verifiable();

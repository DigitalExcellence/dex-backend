...
namespace Services.ExternalDataProviders
{
    ...
    public class GithubDataSourceAdaptee : IAuthorizedDataSourceAdaptee, IPublicDataSourceAdaptee
    {
        ...
        {
            GithubDataSourceResourceResult[] resourceResults =
                (await FetchAllPublicGithubRepositories(username)).ToArray();
            if(!resourceResults.Any()) return null;
            List<Project> projects = mapper
                                     .Map<IEnumerable<GithubDataSourceResourceResult>, IEnumerable<Project>>(
                                         resourceResults)
                                     .ToList();
            return projects;
        }

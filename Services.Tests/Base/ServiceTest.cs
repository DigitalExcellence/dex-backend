...
namespace Services.Tests.Base
{
        ...
        where TRepository : class, IRepository<TDomain>
    {
        ...
        {
            RepositoryMock.Setup(repository => repository.FindAsync(1))
                          .Returns(Task.FromResult(entity));

            TDomain retrievedEntity = await Service.FindAsync(1);

            Assert.DoesNotThrow(() => { RepositoryMock.Verify(repository => repository.FindAsync(1), Times.Once); });
        ...
        {
            RepositoryMock.Setup(repository => repository.Add(entity));

            Service.Add(entity);

            Assert.DoesNotThrow(() => { RepositoryMock.Verify(repository => repository.Add(entity), Times.Once); });
        }
        ...
        {
            RepositoryMock.Setup(repository => repository.AddRange(entities));
        ...
        {
            RepositoryMock.Setup(repository => repository.Update(entity));

            Service.Update(entity);

            Assert.DoesNotThrow(() => { RepositoryMock.Verify(repository => repository.Update(entity), Times.Once); });
        }
        ...
        {
            RepositoryMock.Setup(repository => repository.Remove(entity));

            Service.Remove(entity);

            Assert.DoesNotThrow(() => { RepositoryMock.Verify(repository => repository.Remove(entity), Times.Once); });
        }
        ...
        {
            RepositoryMock.Setup(repository => repository.RemoveAsync(id));

            await Service.RemoveAsync(id);

            Assert.DoesNotThrow(() => { RepositoryMock.Verify(repository => repository.RemoveAsync(id), Times.Once); });
        }
        ...
        public virtual async Task GetAll(List<TDomain> entities)
        {
            ...
            RepositoryMock.Setup(repository => repository.GetAll())
                          .Returns(Task.FromResult((IEnumerable<TDomain>) entities));

            List<TDomain> retrievedList = (List<TDomain>) await Service.GetAll();

            Assert.DoesNotThrow(() => { RepositoryMock.Verify(repository => repository.GetAll(), Times.Once); });
        ...
        {
            RepositoryMock.Setup(repository => repository.Save());

            Service.Save();

            Assert.DoesNotThrow(() => { RepositoryMock.Verify(repository => repository.Save(), Times.Once); });
        }

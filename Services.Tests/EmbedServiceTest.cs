...
namespace Services.Tests
{
    ...
    public class EmbedServiceTest : ServiceTest<EmbeddedProject, EmbedService, IEmbedRepository>
    {
        ...
        ///     The service.
        /// </value>
        protected new IEmbedService Service => base.Service;
        ...
        {
            RepositoryMock.Setup(repository => repository.IsNonExistingGuidAsync(project.Guid))
            ...
            Assert.DoesNotThrow(() =>
            {
                RepositoryMock.Verify(repository => repository.IsNonExistingGuidAsync(project.Guid),
                                      Times.Once);
            });
        ...
        [Test]
        public async Task GetEmbeddedProjects_goodflow(
            [EmbeddedDataSource(100)] IEnumerable<EmbeddedProject> embeddedProjects)
        {
            RepositoryMock.Setup(repository => repository.GetEmbeddedProjectsAsync())
                          .Returns(Task.FromResult(embeddedProjects));

            IEnumerable<EmbeddedProject> actualEmbeddedProjects = await Service.GetEmbeddedProjectsAsync();

            Assert.DoesNotThrow(() =>
            {
                RepositoryMock.Verify(repository => repository.GetEmbeddedProjectsAsync(),
                                      Times.Once);
            });
        ...
        {
            RepositoryMock.Setup(repository => repository.GetEmbeddedProjectsAsync())
                          .Returns(Task.FromResult(Enumerable.Empty<EmbeddedProject>()));
        ...
        {
            RepositoryMock.Setup(repository => repository.GetEmbeddedProjectAsync(project.Guid))
        ...
        {
            RepositoryMock.Setup(repository => repository.GetEmbeddedProjectAsync(project.Guid))
                          .Returns(Task.FromResult((EmbeddedProject) null));
